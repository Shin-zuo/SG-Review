...
@section('title', 'Edit Course | SG-Review')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Edit: {{ $course->acronym }}</h2>
            <p class="text-slate-500 text-sm">Update the details for this reviewer course.</p>
        </div>
    </div>
        ...
        <div class="p-6 md:p-8 space-y-6">
            <div class="grid md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" id="title" name="title" required value="{{ old('title', $course->title) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>

                <div class="space-y-2">
                    <label for="acronym" class="block text-sm font-bold text-slate-700">Acronym <span class="text-red-500">*</span></label>
                    <input type="text" id="acronym" name="acronym" required value="{{ old('acronym', $course->acronym) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <input type="url" id="enrollment_link" name="enrollment_link" required value="{{ old('enrollment_link', $course->enrollment_link) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none">
                    <p class="text-[11px] text-slate-500">Used as fallback if payment gateways or server are under maintenance.</p>
                </div>

                <div class="space-y-2 sm:col-span-2">
                    <label for="google_classroom_id" class="block text-sm font-bold text-slate-700">Google Classroom ID / Invite Code <span class="text-slate-400 font-normal">(Optional / For automatic invite)</span></label>
                    <input type="text" id="google_classroom_id" name="google_classroom_id" value="{{ old('google_classroom_id', $course->google_classroom_id ?? '') }}" placeholder="e.g. https://classroom.google.com/c/... or 12345678" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                    <p class="text-[11px] text-slate-500">Used for automatic enrollment link when Xendit payment/trial succeeds.</p>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-6 border-t border-slate-100 pt-6">
                <div class="space-y-3">
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        @php
                            $colors = ['slate-800' => 'bg-slate-800', 'blue-600' => 'bg-blue-600', 'cyan-500' => 'bg-cyan-500', 'emerald-600' => 'bg-emerald-600', 'purple-600' => 'bg-purple-600', 'rose-600' => 'bg-rose-600'];
                            $selectedColor = old('bg_color', $course->bg_color);
                        @endphp
                        ...
                        <label class="cursor-pointer group relative">
                            ...
                            <div class="w-9 h-9 rounded-full {{ $bgClass }} ring-2 ring-offset-2 ring-transparent peer-checked:ring-slate-400 hover:scale-110 shadow-sm border border-black/10"></div>
                        </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="badge" name="badge" value="{{ old('badge', $course->badge) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                    ...
                    @if($course->image_path)
                        <div class="mb-3 p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-between shadow-sm">
                            <div class="flex items-center gap-3">
                                <img src="{{ asset('storage/' . $course->image_path) }}" class="w-10 h-10 rounded object-cover border border-slate-300">
                                <span class="text-sm text-slate-600 font-medium">Current Image</span>
                            </div>
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input type="checkbox" name="remove_image" value="1" class="w-4 h-4 text-red-600 rounded border-slate-300 focus:ring-red-500 cursor-pointer">
                                <span class="text-sm font-bold text-slate-500 group-hover:text-red-600 transition-colors">Remove</span>
                            </label>
                        </div>
                    @endif
