        ...
        @foreach ($courses as $course)
            <div id="detailsModal-{{ $course->id }}"
                ...
                <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"
                    onclick="closeModal('detailsModal-{{ $course->id }}')"></div>
                            ...
                            @forelse($course->modules as $module)
                                <div
                                    ...
                                    <div class="flex items-center gap-3 mb-3">
                                        <div
                                            class="w-10 h-10 rounded-xl bg-blue-100 text-blue-700 font-bold flex items-center justify-center shrink-0">
                                        ...
                                        <ul class="mt-4 ml-5 space-y-3 border-l-2 border-slate-100 pl-5">
                                            @foreach ($module->lessons as $lesson)
                                                <li class="flex items-start gap-3 text-slate-600">
                                                    <svg class="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" fill="none"
                                                    ...
                                                    <div class="leading-snug">
                                                        <span class="font-semibold text-slate-700">Lesson
                                                            {{ $lesson->lesson_number }}:</span>
                                                        {{ $lesson->lesson_title }}
                                                    </div>
                                                </li>
                                            @endforeach
                                    ...
                                    @else
                                        <p class="text-sm text-slate-400 italic mt-2 ml-14">No lessons added to this module
                                            yet.</p>
                                    @endif
                                </div>
                            @empty
                                <div class="text-center py-10 bg-white rounded-2xl border border-slate-200 border-dashed">
                                    <div
                                        class="w-16 h-16 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center mx-auto mb-3">
                                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
