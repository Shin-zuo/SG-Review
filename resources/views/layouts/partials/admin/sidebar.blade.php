<aside id="adminSidebar" class="w-60 bg-slate-900 text-slate-300 flex flex-col fixed inset-y-0 left-0 z-30 transform -translate-x-full md:relative md:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl md:shadow-none">
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        ...
        <a href="{{ route('dashboard') }}" 
           ...
           {{ request()->routeIs('dashboard') ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
        ...
        <a href="{{ route('analytics') }}" 
           class="flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium transition-all
           {{ request()->routeIs('analytics*') ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
        ...
        <a href="{{ route('reviewers') }}" 
           class="flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium transition-all
           {{ request()->routeIs('reviewers*') ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
    ...
    <div class="p-4 border-t border-slate-800">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="flex items-center gap-3 text-sm font-medium text-red-400 hover:text-red-300 transition-colors w-full px-3 py-2 bg-transparent border-none cursor-pointer">
